/**
 * Stat import from cyberpunkred.com export data.
...
function getStatsV1(data) {
  return data.stat.map((stat) => [
    STAT_TYPE_MAP[stat.stat_type_id],
    stat.points,
  ]);
}
...
export async function updateStats(data, actor, isV2) {
  const stats = (isV2 ? getStatsV2(data) : getStatsV1(data)).reduce(
    (acc, [statName, value]) => {
      acc[statName] = { value };
