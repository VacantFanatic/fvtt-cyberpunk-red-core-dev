/**
 * Skill level import from cyberpunkred.com export data.
  ...
  return data.skill.map(async (skillData) => {
    ...
    if (skillItem) {
      ...
      ui.notifications.warn(
        game.i18n.format("CPR.import.cyberpunkredCom.skillNotFound", {
          name: skillName,
        })
      );
    }
...
function updateSkillsV2(data, actor) {
  return Object.entries(data.skills).map(async ([skillName, level]) => {
    ...
    if (skillItem) {
      ...
      ui.notifications.warn(
        game.i18n.format("CPR.import.cyberpunkredCom.skillNotFound", {
          name: SKILL_NAME_MAP_V2[skillName] ?? skillName,
        })
      );
    }
  });
...
export async function updateSkills(data, actor, isV2) {
  const skillUpdates = isV2
    ? updateSkillsV2(data, actor)
    : updateSkillsV1(data, actor);
