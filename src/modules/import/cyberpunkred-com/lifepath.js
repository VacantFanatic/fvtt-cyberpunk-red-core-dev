/**
 * Lifepath import from cyberpunkred.com export data.
...
// eslint-disable-next-line import/prefer-default-export -- named export matches importer modules
export async function updateLifepath(data, actor) {
  const identifyingFeatures = (
    data.identifying_features ?? data.identifyingFeatures
  ).split("\n\n");
  const background = data.background.split("\n\n");
  const roleLifePath = [];
  const lifepath = [...identifyingFeatures, ...background].reduce(
    (acc, item) => {
      const bits = item.split("\n");
      const label = bits[0];
      const match = Object.entries(LIFEPATH_FIELDS).find(([, regex]) =>
        regex.test(label)
      );
      if (match) {
      ...
      const roleMatch = Object.entries(LIFEPATH_ROLE_FIELDS).find(([, regex]) =>
        regex.test(label)
      );
      if (roleMatch) {
  ...
  const contacts = data.contact || Object.values(data.contacts ?? {});
  ...
  new Set(Object.values(CONTACT_RELATIONSHIP_TYPES)).forEach((name) => {
    if (lifepath[name]) {
      lifepath[name] = lifepath[name].join("\n\n");
    }
  });

  Object.keys(lifepath).forEach((key) => {
    lifepath[key] = (lifepath[key] ?? "").replaceAll("\n", "<br>");
  });

  const system = {};
  if (
    data.character_type_id === 0 ||
    data.characterType === "PlayerCharacter"
  ) {
    ...
    ui.notifications.warn(
      game.i18n.format("CPR.import.cyberpunkredCom.unknownCharacterType", {
        type: data.character_type_id ?? data.characterType,
        name: data.name,
      })
    );
  }
