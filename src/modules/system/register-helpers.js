...
export default function registerHandlebarsHelpers() {
  ...
  Handlebars.registerHelper("cprWikiLink", (string) => {
    const repoUrl = `https://github.com/cyberpunk-red-team/fvtt-${game.system.id}`;
    const wikiPage = (slug) => `${repoUrl}/wiki/${slug}`;
    const itemPath = "System-Documentation-Items";
    switch (string) {
      case "ammo":
        return wikiPage(`${itemPath}-Ammo`);
      case "armor":
        return wikiPage(`${itemPath}-Armor`);
      case "clothing":
        return wikiPage(`${itemPath}-Clothing`);
      case "criticalInjury":
        return wikiPage(`${itemPath}-Critical-Injuries`);
      case "cyberdeck":
        return wikiPage(`${itemPath}-Cyberdecks`);
      case "cyberware":
        return wikiPage(`${itemPath}-Cyberware`);
      case "drug":
        return wikiPage(`${itemPath}-Drugs`);
      case "gear":
        return wikiPage(`${itemPath}-Gear`);
      case "itemUpgrade":
        return wikiPage(`${itemPath}-Upgrades`);
      case "netarch":
        return wikiPage(`${itemPath}-NET-Architecture`);
      case "program":
        return wikiPage(`${itemPath}-Programs`);
      case "role":
        return wikiPage(`${itemPath}-Roles`);
      case "skill":
        return wikiPage(`${itemPath}-Skills`);
      case "vehicle":
        return wikiPage(`${itemPath}-Vehicles`);
      case "weapon":
        return wikiPage(`${itemPath}-Weapons`);
      default:
        return string;
    }
  });
