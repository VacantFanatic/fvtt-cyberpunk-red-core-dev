...
- Cursor Cloud update flow uses `scripts/cloud-update.sh` and `scripts/ensure-docker-cli.sh` to bootstrap the Docker CLI against the host daemon at `tcp://127.0.0.1:2375`.
- `foundry-docker.sh` accepts `FOUNDRY_USER` / `FOUNDRY_ACCOUNT_PASSWORD` as credential secret aliases.
...
- Restored `docker/foundry-compose.yml` Foundry service definition accidentally removed in v1.4.1 (empty `services:` block prevented Docker Foundry from starting).
...
- Role ability settings were lost when changing other data on the sheet. [issue #203](https://github.com/cyberpunk-red-team/fvtt-cyberpunk-red-core/issues/203)
